...
/**
 * @param {string|{start: string, end: string, prefix: string, suffix: string}} blockFromOptions
 *
...
function prepareBlock(blockFromOptions) {
  ...
  if (typeof blockFromOptions === 'string') {
    block = generateBlockFromString(blockFromOptions, blockFromOptions);
  }
...
function generateBlockFromObject(block) {
  const defaults = generateBlockFromString(block.start, block.end);
